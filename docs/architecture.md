...
| **HDD-Music** | Western Digital Green (`WD20EZRX-00DC0B0`) | 3.5" SATA III (2TB) | `WD-WCC1T0899623` | `/mnt/hdd-music` (device letter now floats — currently `sdd1`, was `sdc1`; identify by serial, not letter) | **Active, healthy.** 2026-09-19: had a scare where it looked like drive failure (`IDENTIFY` handshake fails under load) — full investigation in `CHANGELOG.md` (40)/(42) ruled out the M.2 SATA card, port, and power cable, initially suspected the drive itself, but a `smartctl` deep-dive confirmed **0 reallocated/pending/uncorrectable sectors and `PASSED` overall-health** — root cause was actually a degrading SATA data cable (real-time SATA Phy CRC counters read 0 post-cable-swap). Drive is safe for normal read/write use. 717GB music collection intact throughout. |
