...
| Movies, TV, Anime, Manga, Torrents, JDownloader | HDD-Media (`/mnt/hdd-media/`) | ext4, `Seagate Barracuda 1TB 7200 RPM` (sdd2) | ~679G / 916G (74%) |
| Nextcloud, Syncthing, LAN Shared | HDD-Cloud (`/mnt/hdd-cloud/`) | ext4, `Toshiba 1TB 2.5"` (sdb2) | 105G (own data) + 717G (rescued music) / 916G |
...
| **HDD-Music (now HDD-Backup role)** | Western Digital Green (`WD20EZRX-00DC0B0`) | 3.5" SATA III (2TB) | `WD-WCC1T0899623` | `/mnt/hdd-music` (device letter now floats — currently `sde1`; identify by serial, not letter) | **⚠️ UNSTABLE hardware, but role finalized: cold-backup only.** Root cause (per (56)/(59)): the drive's own controller/firmware intermittently fails the ATA `IDENTIFY` handshake and NCQ command processing — confirmed not the cable (link layer clean, SMART `PASSED`) and not the platters (0 bad sectors). Fails under NCQ-heavy random/small-file I/O (torrent writes, SMB scans); tolerates large sequential transfers fine (the (57)-(59) 717GB rescue ran ~15hrs without a hard drop). **2026-09-21: user decided not to wait further on the pending RMA conversation and finalized this drive's role as cold-backup-only**, matching the access pattern it actually tolerates. Remounted rw at its proper `/mnt/hdd-music` path (was on a temporary `/mnt/hdd-music-rescue` mountpoint during the rescue), `docker-host` (LXC 100) rebooted to pick up the live bind mount again (all 20 containers came back healthy, verified NPM/checklist/portfolio still serving correctly post-reboot). `scripts/backup.sh`'s existing `/mnt/hdd-music/backups` fallback path now applies as designed. **717GB of music still physically present here** (the original copy rescue was read from) — redundant with the canonical copy now served from `hdd-cloud`, left in place as a free extra backup rather than deleted. RMA conversation with the seller can still proceed independently/at user's leisure — a successful RMA would mean a healthy replacement drive takes over this cold-backup role with a much wider safe-usage envelope, but isn't blocking anything anymore. |
...
| **HDD-Cloud** | Toshiba 2.5" HDD (`MQ04ABF100`) | 2.5" SATA III 5400 RPM (1TB) | `Y9CSTR0WT` | `/mnt/hdd-cloud` (sdb2) | **Active.** Quiet/low-power: Nextcloud, Syncthing, LAN Shared folder, **+ music library** (`music-rescue/`, added 2026-09-21, now what Navidrome/Jellyfin actually serve — see `hdd-music`'s row for why). |
