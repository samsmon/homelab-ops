...
| **HDD-Music** | Western Digital Green (`WD20EZRX-00DC0B0`) | 3.5" SATA III (2TB) | `WD-WCC1T0899623` | `/mnt/hdd-music` (device letter now floats — currently `sdd1`, was `sdc1`; identify by serial, not letter) | **Failing, mounted read-only.** 2026-09-19: drive started failing `IDENTIFY` handshake under load (SATA link fine, drive controller not responding to commands) — see `CHANGELOG.md` (40) for the full investigation (cable/port/M.2 card/power all ruled out, isolated to the drive itself). Currently accessible read-only with the 723GB `music/` library intact, but hard-resets under sustained I/O — treat as failing, not fixed. Do not mount read-write or run heavy I/O against it until replaced. |
