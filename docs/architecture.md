...
| **HDD-Music** | Western Digital Green (`WD20EZRX-00DC0B0`) | 3.5" SATA III (2TB) | `WD-WCC1T0899623` | `/mnt/hdd-music` (device letter now floats — currently `sdd1`, was `sdc1`; identify by serial, not letter) | **⚠️ UNSTABLE, recurring drop — physical check pending.** 2026-09-19: first scare (`IDENTIFY` handshake fails under load) — `CHANGELOG.md` (40)/(42) ruled out the M.2 SATA card/port/power cable, `smartctl` showed `PASSED`/0 reallocated sectors, concluded root cause was a degrading SATA data cable and declared the drive safe for rw use. **2026-09-20 (55): dropped from the kernel again** with the identical `ata9.00: failed to IDENTIFY` signature, this time under a read-only workload (SMB library scan + playback), not heavy writes — so the (40)/(42) "fixed" verdict is reopened, not confirmed. Device currently gone from `lsblk`/`udev` entirely; deliberately left alone (no remount/reboot attempted) since reconnecting it in a bad state can hang `pve` at BIOS/POST for 5+ min. **Do not assume rw-safe until user reseats/re-checks the physical SATA cable+port.** 717GB music collection intact as of last check. |
