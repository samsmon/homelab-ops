...
| **HDD-Backup (WD Blue 320GB, separate/unrelated 4th drive)** | Western Digital Blue | 3.5" SATA (320GB) | - | `/mnt/hdd-backup` | **Pending.** Standby for 4th external dock solution. **Naming collision to flag**: the user has also been referring to the old `hdd-music` (WD Green 2TB) drive's new cold-backup role as "hdd-backup" in conversation — that's a *different* physical drive from this WD Blue 320GB one. Needs disambiguating before any actual rename happens (e.g. this one could become `hdd-backup2`/`hdd-backup-small`, or the WD Green's role gets a different name like `hdd-archive`). |
