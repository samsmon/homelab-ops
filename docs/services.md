...
| T3 Code | Agent harness control surface for Claude Code, Antigravity/Gemini, etc. | 9001 (docker-host) | **docker-host**: Primary active instance (`t3code` on port 9001, `configs/docker-compose/t3code.yml`) with persistent named volume `docker-compose_t3code_data` mounted at `/root`. Consolidated back to single primary instance on 2026-09-13 (`t3code-2` and `t3code-3` decommissioned). Custom Dockerfile bundles `@anthropic-ai/claude-code` + `t3` CLI. T3 Connect pairing tokens minted with `npx t3 pair --ttl 168h` (7 days) and bound to `t3.suryatmaja.dev` via Cloudflare Tunnel. **Resource-limited 2026-09-14** (`mem_limit: 3g`, `cpus: 2.0`) after repeated `Git command timed out` errors (`fetchRemoteForStatus`/`branchPullRequest`, worst offender: `homelab-ops` itself, 42 occurrences) left agent threads stuck "working" with no progress — container previously had no limit at all and accumulated 379 PIDs incl. zombies from unreaped `node`/`localharness_ex` children. If timeouts return, check host-wide CPU contention first (another container in a crash-loop can starve git subprocesses past their internal timeout) before assuming a t3code-specific bug. |
