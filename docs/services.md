...
| AGY Web Terminal (ttyd + tmux) | Dedicated persistent web terminal & multi-terminal workspace for Antigravity / Claude Code | 7681 | **Deployed 2026-09-15 on `dev-host` (LXC 102, `192.168.18.227:7681`).** Managed by systemd service `ttyd.service`. Directly connects to persistent `tmux` session `agy-workspace` rooted at `/workspace`. Publicly accessible via Cloudflare Tunnel at `https://agy.suryatmaja.dev`. Supports multi-window/session AI orchestrations and isolated account profiles. |
